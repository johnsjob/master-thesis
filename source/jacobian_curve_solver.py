...
LEGEND_SIZE = PlotSettings.legend_size * 1.25
LABEL_SIZE  = PlotSettings.label_size  * 1.0
TICK_SIZE   = PlotSettings.tick_size   * 1.0
...
def main():
    ...
    yticks(fontsize=TICK_SIZE)
    xlim((0,49))
    legend(['$\dot{q}_'+str(index+1)+'$'], loc='lower right', fontsize=LEGEND_SIZE)
    maximize_plot()
    ...
    yticks(fontsize=TICK_SIZE)
    xlim((0,49))
    legend(['$\dot{q}_'+str(index+1)+'$'], loc='upper right', fontsize=LEGEND_SIZE)
    maximize_plot()
    ...
    yticks(fontsize=TICK_SIZE)
    xlim((0,49))
    legend(['$q_'+str(index+1)+'$'], loc='lower right', fontsize=LEGEND_SIZE)
    subplot(212)
    custom_plot(n.round(joint_angular_vel[:,index],decimals=3), 'r', linewidth=1.5)
    xlabel(XLABEL, fontsize=LABEL_SIZE)
    ylabel(YLABEL_W, fontsize=LABEL_SIZE)
